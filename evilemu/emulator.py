...
class Emulator(abc.ABC):
    ...
    def __init__(self, process: ProcessBase, rom_address: int, ram_address: int, hram_address: int):
        self.__process = process
        self.__rom_address = rom_address
        self.__ram_address = ram_address
        self.__hram_address = hram_address
    ...
    def read_hram(self, offset: int, size: int) -> bytes:
        return self.__process.read_memory(self.__hram_address + offset, size)
    ...
    def write_hram(self, offset: int, data: bytes) -> None:
        return self.__process.write_memory(self.__hram_address + offset, data)
    ...
    def read_hram8(self, offset: int) -> int:
        return int.from_bytes(self.read_hram(offset, 1), 'little')
    ...
    def write_hram8(self, offset: int, data: int) -> None:
        return self.write_hram(offset, data.to_bytes(1, 'little'))
    ...
    def read_hram16(self, offset: int) -> int:
        return int.from_bytes(self.read_hram(offset, 2), 'little')
    ...
    def write_hram16(self, offset: int, data: int) -> None:
        return self.write_hram(offset, data.to_bytes(2, 'little'))
